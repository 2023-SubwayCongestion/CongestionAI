...
# A potential smaller regression model
class StudentRegressionModel(nn.Module):
    # feature size가 기존에는 256이었음
    def __init__(self, num_features_in, num_anchor_points=4, feature_size=128):
        super(StudentRegressionModel, self).__init__()
...
# A potential smaller classification model
class StudentClassificationModel(nn.Module):
    # feature size가 기존에는 256이었음
    def __init__(self, num_features_in, num_anchor_points=4, num_classes=80, feature_size=128):
        super(StudentClassificationModel, self).__init__()
...
# the defenition of the P2PNet student model
class P2PNet_Student(nn.Module):
    def __init__(self, backbone, row=2, line=2):
        ...
        self.regression = StudentRegressionModel(
            num_features_in=256, num_anchor_points=num_anchor_points)
        self.classification = StudentClassificationModel(num_features_in=256,
...
# create the P2PNet student model
def build(args, training):
    # treats persons as a single class
    num_classes = 1

    backbone = build_backbone(args)
    model = P2PNet_Student(backbone, args.row, args.line)
    if not training:
        return model
