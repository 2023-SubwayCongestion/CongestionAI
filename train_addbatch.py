...
def tensor_to_img(tensor):
    # Convert tensor from BxCxHxW to BxHxWxC
...
def teacher_density_map(img, teacher_points):
    ...
    img: input image (batched)
    teacher_points: a list of pedestrian locations in the format [[col, row], [col, row], ...] (batched)

    return:
    batch_density: the batch of density maps corresponding to the given points. It has the same shape as the input image. (batched)
    """
    batch_size = teacher_points.shape[0]
    img_shape = [img.shape[1], img.shape[2]]  # Assuming img is also batched [B, H, W, C]
    
    # Initialize a batch of density maps
    batch_density = np.zeros((batch_size, *img_shape), dtype=np.float32)
    
    for i in range(batch_size):  # Loop over the batch size
        single_teacher_points = teacher_points[i].cpu().numpy()
        
        # Initialize density map for this item in the batch
        density = np.zeros(img_shape, dtype=np.float32)
        
        # If there are no points, continue to the next item in the batch
        if len(single_teacher_points) == 0:
            continue

        leafsize = 2048
        # Build KDTree for the given points
        tree = scipy.spatial.KDTree(single_teacher_points.copy(), leafsize=leafsize)
        
        # Query KDTree for distances
        distances, _ = tree.query(single_teacher_points, k=4)

        for j, pt in enumerate(single_teacher_points):
            pt2d = np.zeros(img_shape, dtype=np.float32)
            ...
            # If more than one point, use average of three smallest distances as sigma
            if len(single_teacher_points) > 1:
        ...
        # Store the density map in our batch of density maps
        batch_density[i] = density
...
if __name__=="__main__":
    ...
    #optimizer = torch.optim.SGD(mcnn.parameters(), lr=1e-6,
                                #momentum=0.95)
    optimizer = torch.optim.Adam(mcnn.parameters(), lr=1e-6, betas=(0.9, 0.999))
