...
if __name__ == "__main__":
    ...
    test_dataset = CrowdDataset(
        test_img_root, test_gt_dmap_root, 4, dataset_name="SHA", is_train=False
    )
    ...
    for epoch in tqdm(range(2000), desc="Processing epochs"):
        ...
        for i, (img, gt_dmap) in tqdm(
            ...
            to_density_map = teacher_density_map(img_np, teacher_dmap["pred_points"])
            teacher_dmap = (
                torch.tensor(to_density_map).unsqueeze(0).unsqueeze(0).to(device)
            )
            et_dmap = F.interpolate(
                et_dmap, size=(400, 400), mode="bilinear", align_corners=False
            )
        ...
        vis.image(
            ...
            opts=dict(title="et_dmap(" + str(et_dmap.sum()) + ")"),
        )
